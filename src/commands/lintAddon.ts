...
export async function lintAddonLocally() {
  ...
  if (!fsInput) {
    vscode.window.showErrorMessage(
      "lintAddonLocally failed, no input provided"
    );
  ...
  if (!existsSync(fsInput)) {
    vscode.window.showErrorMessage(
      "lintAddonLocally failed, the file path doesn't exist in local file system"
    );
    throw new Error(
      "lintAddonLocally failed, the file path doesn't exist in local file system!"
    );
  ...
  try {
    ...
    console.log(
      `Summary: [Warnings: ${lintResults.summary.warnings.toString()}], [Errors: ${lintResults.summary.errors.toString()}], [Notices: ${lintResults.summary.notices.toString()}]`
    );
    ...
    if (i > 0) {
      for (i = 0; i < lintResults.errors.length; i++) {
        ...
        message = `Message {_type: ${lintResults.errors[
          i
        ]._type.toString()}, message: ${lintResults.errors[
          i
        ].message.toString()}, description: ${lintResults.errors[
          i
    ...
    if (i > 0) {
      for (i = 0; i < lintResults.warnings.length; i++) {
        ...
        message = `Message {_type: ${lintResults.warnings[
          i
        ]._type.toString()}, message: ${lintResults.warnings[
          i
        ].message.toString()}, description: ${lintResults.warnings[
          i
        ].description.toString()}}`;
        data += "\n" + message;
      }
    ...
    if (i > 0) {
      for (i = 0; i < lintResults.notices.length; i++) {
        ...
        message = `Message {_type: ${lintResults.notices[
          i
        ]._type.toString()}, message: ${lintResults.notices[
          i
        ].message.toString()}, description: ${lintResults.notices[
          i
        ].description.toString()}}`;
        data += "\n" + message;
      }
  ...
  const dest = fs.createWriteStream(
    `${workspaceFolder}/${fileName}_lintResults.txt`,
    { flags: "w" }
  );
  dest.write(data);
}
...
export async function lintAddonApi() {
  const fsInput = await vscode.window.showInputBox({
    // TODO: What input could uniquely identify the extension?
  ...
  try {
    ...
    console.log(
      "versionInfo:" + versionInfo.fileID + " " + versionInfo.version
    );
