...
let commentCacheControllerStub: sinon.SinonStubbedInstance<CommentCacheController>,
...
const cmt = {
  uri: vscode.Uri.file("test-root/test-guid/test-version/test-filepath"),
};
...
describe("CommentController.ts", () => {
  ...
  describe("deleteThread", () => {
    it("should delete the comment thread from a controller and its comments from cache.", async () => {
      ...
      expect(commentCacheControllerStub.deleteCommentFromCache.called).to.be
        .true;
    });
  });
  ...
  describe("deleteCommentsFromMenu()", () => {
    ...
    it("delete the comments of given uris.", async () => {
      ...
      expect(commentCacheControllerStub.deleteComments.calledWith(uriOne)).to.be
        .true;
      expect(commentCacheControllerStub.deleteComments.calledWith(uriTwo)).to.be
        .true;
      expect(commentCacheControllerStub.deleteComments.calledWith(uriThree)).to
        .be.true;
      expect(result).to.have.lengthOf(0);
    });
  });
  ...
  describe("exportComments()", () => {
    it("call to export comments from cache and activate a new controller with re-fetched comments in place of the old one.", async () => {
      const exportVersionCommentsStub =
        commentCacheControllerStub.exportVersionComments.resolves(true);
