...
import { AddonTreeDataProvider, AddonTreeItem } from "../../../src/model/sidebarTreeDataProvider";
...
describe("fileDecorationProvider.ts", async () => {
  beforeEach(() => {
    ...
    sinon.stub(fs, 'statSync').callsFake((filePath: fs.PathLike) => ({
    ...
    describe("sidebarTreeDataProvider.ts", async () => {
        ...
        describe("getTreeItem", () => {
            it("should return the element passed to it.", () => {
                const item = new AddonTreeItem("file", vscode.Uri.parse("path/to/file"));
        ...
        describe("getChildren", () => {
            ...
            const fakeFiles = ["guidDir1", "junk1", "guidDir2", "guidDir3", "junk2"];
            ...
            it("should correctly get the directory children of the guid folder, the downloaded versions.", async () => {
                const fakeFiles = ["version1Dir", "junk1", "version2Dir", "junk2", "version3Dir", "version4Dir"];
                sinon.stub(fs, "readdir").yields(null, fakeFiles);
                
                const item = new AddonTreeItem("guid1", vscode.Uri.parse("test-root/guid1"), true);
            ...
            it("should correctly return NO children of the version folder, even if they exist.", async () => {
                const fakeFiles = ["file1", "junk1", "file2", "junk2", "file3", "file4Dir"];
                sinon.stub(fs, "readdir").yields(null, fakeFiles);
                
                const item = new AddonTreeItem("version1", vscode.Uri.parse("test-root/guid1/version1"));
                const children = await addonTreeDataProvider.getChildren(item);
